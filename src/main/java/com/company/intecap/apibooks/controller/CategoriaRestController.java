...
public class CategoriaRestController {
    ...
    public ResponseEntity<CategoriaResponseRest> guardarCategoria(@Valid @RequestBody Categoria request, BindingResult bindingResult){
        if(bindingResult.hasErrors()){ // Si hay errores en la validación de los datos
            response.setMetadata("Respuesta no ok", "500", Objects.requireNonNull(bindingResult.getFieldError()).getDefaultMessage());
